...
revision() {
	...
	if git_is_file_tracked "$1" ; then
		local sha=$(git_last_commit "$1")
		local lasttag=$(git describe --abbrev=0 "$sha")
		local cnt=$(git rev-list ${lasttag}..${sha} --count)
		local branch=$(git name-rev --name-only ${sha})
		if [ -n "${lasttag}" -a -n "${cnt}" -a -n "${branch}" -a -n "${sha}" ]; then
			r="${lasttag}-${cnt}-${branch}-${sha}"
